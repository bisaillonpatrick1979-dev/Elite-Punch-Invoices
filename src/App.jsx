...
const allTabs = [
  ...
  { id: "catalog", icon: "◫", component: Catalog, roles: ["owner"], showInNav: false },
  { id: "accounting", icon: "▦", component: Accounting, roles: ["owner"], showInNav: false },
  { id: "devtools", icon: "🛠", component: DevTools, roles: ["owner"], showInNav: false }
];
...
export default function App() {
  ...
  const routeTabs = useMemo(() => allTabs.filter((tab) => tab.roles.includes(mode)), [mode]);
  const navTabs = useMemo(() => routeTabs.filter((tab) => tab.showInNav), [routeTabs]);
  const currentTab = useMemo(() => routeTabs.find((tab) => tab.id === activeTab) || routeTabs[0], [activeTab, routeTabs]);
  ...
  return (
    <div className="app-shell" data-theme={theme}>
      ...
      <main className="app-main"><AppStatus /><ActiveComponent t={t} language={language} setLanguage={setLanguage} theme={theme} setTheme={setTheme} themes={themes} onNavigate={setActiveTab} /></main>
      <nav className="bottom-tabs scroll-tabs" aria-label="Navigation principale">{navTabs.map((tab) => { const isActive = tab.id === currentTab.id; const label = getTabLabel(tab, t); return <button key={tab.id} type="button" className={isActive ? "tab-button active" : "tab-button"} onClick={() => setActiveTab(tab.id)} aria-current={isActive ? "page" : undefined}><span className="tab-icon">{tab.icon}</span><span className="tab-label">{label}</span></button>; })}</nav>
    </div>
  );
}

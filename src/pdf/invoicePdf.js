...
export function safeInvoiceFileName(value, fallback = "invoice") {
...
export function createInvoicePdfDoc({ invoice, settings }) {
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  [safeText(invoice.clientName, "No client"), safeText(invoice.jobAddress, "No job address"), invoice.clientPhone ? `Phone: ${invoice.clientPhone}` : "", invoice.clientEmail ? `Email: ${invoice.clientEmail}` : ""].filter(Boolean).forEach((line, index) => doc.text(line, 14, 69 + index * 5));

  autoTable(doc, { startY: 94, head: [["Description", "Qty", "Unit", "Rate", "Total"]], body: (invoice.lines || []).map((line) => [safeText(line.description), Number(line.quantity || 0).toFixed(2), safeText(line.unit), formatMoney(line.rate || 0, currency, locale), formatMoney(line.total || 0, currency, locale)]), styles: { font: "helvetica", fontSize: 9, cellPadding: 3 }, headStyles: { fillColor: [20, 20, 20], textColor: [255, 255, 255] } });
...
export function getInvoicePdfFile({ invoice, settings }) {
  const doc = createInvoicePdfDoc({ invoice, settings });
  const blob = doc.output("blob");
  const fileName = `${safeInvoiceFileName(invoice.invoiceNumber)}.pdf`;
...
export function downloadInvoicePdf({ invoice, settings }) {
  const doc = createInvoicePdfDoc({ invoice, settings });
  doc.save(`${safeInvoiceFileName(invoice.invoiceNumber)}.pdf`);
}

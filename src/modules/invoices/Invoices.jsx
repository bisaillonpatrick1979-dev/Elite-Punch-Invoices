  ...
  const updateInvoiceField = (invoiceId, field, value) => {
    updateAppData((currentData) => ({
      ...
      invoices: (currentData.invoices || []).map((invoice) =>
        invoice.id === invoiceId ? recalc({ ...invoice, [field]: value }, currentData) : invoice
      )
    }));
  };
          ...
          {invoices.map((invoice) => {
            ...
            return (
              <div className="info-card" key={invoice.id}>
                ...
                <div className="simple-list">{(invoice.lines || []).map((line) => <div className="list-item" key={line.id}><strong>{line.description}</strong><span>{Number(line.quantity || 0).toFixed(2)} {line.unit} | {money(line.total || 0)}</span></div>)}</div>

...
function getAccountingSummary({ invoices = [], punches = [] }) {
  ...
  const totalSubtotal = sum(invoiceTotals.map((total) => total.subtotal));
  ...
  return (
    <section className="module-page">
      <div className="hero-card">
        <span className="status-pill">Accounting</span>
        <h2>Accounting summary</h2>
        <p>Quick view of invoice revenue, balances, GST/taxes, payroll and rough profit.</p>
      </div>

      <div className="card-grid">
        <div className="stat-card">
          <h3>Total invoiced</h3>
          <p>{money(summary.totalRevenue)}</p>
        </div>

        <div className="stat-card">
          <h3>Balance to collect</h3>
          <p>{money(summary.totalBalanceDue)}</p>
        </div>

        <div className="stat-card">
          <h3>Taxes collected</h3>
          <p>{money(summary.totalTax)}</p>
        </div>

        <div className="stat-card">
          <h3>Payroll total</h3>
          <p>{money(summary.payrollTotal)}</p>
        </div>
        ...
        <div className="stat-card">
          <h3>Rough profit</h3>
          <p>{money(summary.grossProfitBeforeMaterials)}</p>
        </div>
      </div>
      ...
      <div className="info-card">
        ...
        <div className="mini-stats">
          <span>{summary.openInvoiceCount} open invoices</span>
          <span>{summary.paidInvoiceCount} paid invoices</span>
          <span>{punches.length} punches</span>
        </div>
      </div>
            ...
            {invoices.slice(0, 8).map((invoice) => {
              ...
              return (
                <div className="list-item" key={invoice.id}>
                  <strong>{invoice.invoiceNumber} - {invoice.clientName}</strong>
                  <span>Status: {invoice.status}</span>
                  <span>Total: {money(totals.total || 0)} | Balance: {money(totals.balanceDue || 0)}</span>
                </div>
              );
            })}
          </div>

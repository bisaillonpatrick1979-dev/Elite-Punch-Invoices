...
export default function Settings() {
  ...
  const updateBillingField = (field, value) => {
    updateAppData((currentData) => ({ ...currentData, settings: { ...currentData.settings, billingProfile: { ...getBillingProfile(currentData.settings), [field]: value } } }));
  };
  ...
  const handleLogoUpload = (event) => {
    ...
    reader.onload = () => {
      updateAppData((currentData) => ({ ...currentData, settings: { ...currentData.settings, billingProfile: { ...getBillingProfile(currentData.settings), logoDataUrl: reader.result, logoFileName: file.name } } }));
    };
  ...
  const saveSignature = (signatureDataUrl) => {
    updateAppData((currentData) => ({ ...currentData, settings: { ...currentData.settings, billingProfile: { ...getBillingProfile(currentData.settings), signatureDataUrl, signatureDate: new Date().toISOString() } } }));
  };
  ...
  const updateTax = (taxId, field, value) => {
    updateAppData((currentData) => ({ ...currentData, settings: { ...currentData.settings, taxProfile: { ...(currentData.settings.taxProfile || {}), taxes: (currentData.settings.taxProfile?.taxes || []).map((tax) => tax.id === taxId ? { ...tax, [field]: value } : tax) } } }));
  };
  ...
  const addTax = () => {
    ...
    updateAppData((currentData) => ({ ...currentData, settings: { ...currentData.settings, taxProfile: { ...(currentData.settings.taxProfile || {}), taxes: [...(currentData.settings.taxProfile?.taxes || []), newTax] } } }));
    setTaxForm({ name: "", rate: "5" });
  };

  const removeTax = (taxId) => {
    updateAppData((currentData) => ({ ...currentData, settings: { ...currentData.settings, taxProfile: { ...(currentData.settings.taxProfile || {}), taxes: (currentData.settings.taxProfile?.taxes || []).filter((tax) => tax.id !== taxId) } } }));
  };
  ...
  const importBackup = (event) => {
    ...
    reader.onload = () => {
      try {
        setAppData(parseBackupJson(String(reader.result)));
      } catch (error) {

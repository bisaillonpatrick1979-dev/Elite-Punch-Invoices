...
export default function Settings({ onNavigate }) {
  ...
  const relaunchOnboarding = () => { updateSettingsField("onboardingDone", false); logout(); };
  ...
  return (
    <section className="module-page">
      <div className="hero-card"><span className="status-pill">Réglages</span><h2>Centre de contrôle</h2><p>Profil, taxes, signature, sauvegarde et accès aux modules internes cachés de la barre du bas.</p><div className="mini-stats"><span>Version {APP_VERSION}</span><span>{settings.region}</span><span>{settings.currency}</span></div></div>

      <div className="info-card settings-module-hub"><h2>Modules / Gestion</h2><p>Ces sections ne sont plus dans la barre de navigation pour garder l’app propre. Elles restent accessibles ici.</p><div className="settings-shortcut-grid"><button className="settings-shortcut-card" type="button" onClick={() => onNavigate?.("payroll")}><span>$</span><strong>Payes</strong><small>Paye, revenus et statuts</small></button><button className="settings-shortcut-card" type="button" onClick={() => onNavigate?.("clients")}><span>◇</span><strong>Clients</strong><small>Liste et coordonnées</small></button><button className="settings-shortcut-card" type="button" onClick={() => onNavigate?.("employees")}><span>👷</span><strong>Employés</strong><small>Workers, sous-traitants, PIN</small></button><button className="settings-shortcut-card" type="button" onClick={() => onNavigate?.("catalog")}><span>◫</span><strong>Catalogue</strong><small>Matériaux, extras, services</small></button><button className="settings-shortcut-card" type="button" onClick={() => onNavigate?.("accounting")}><span>▦</span><strong>Comptabilité</strong><small>Suivi et rapports</small></button><button className="settings-shortcut-card" type="button" onClick={() => onNavigate?.("devtools")}><span>🛠</span><strong>DevTools</strong><small>Outils techniques</small></button></div></div>

      <div className="info-card"><h2>Local defaults</h2><div className="form-grid"><label className="field"><span>Region</span><input value={settings.region || ""} onChange={(event) => updateSettingsField("region", event.target.value)} /></label><label className="field"><span>Currency</span><select value={settings.currency || "CAD"} onChange={(event) => updateSettingsField("currency", event.target.value)}><option value="CAD">CAD</option><option value="USD">USD</option><option value="EUR">EUR</option><option value="GBP">GBP</option><option value="AUD">AUD</option><option value="NZD">NZD</option></select></label><label className="field"><span>Locale</span><select value={settings.locale || "fr-CA"} onChange={(event) => updateSettingsField("locale", event.target.value)}><option value="fr-CA">fr-CA</option><option value="en-CA">en-CA</option><option value="en-US">en-US</option><option value="fr-FR">fr-FR</option></select></label><label className="field"><span>Time zone</span><input value={settings.timeZone || "America/Edmonton"} onChange={(event) => updateSettingsField("timeZone", event.target.value)} /></label></div><div className="action-row"><button className="secondary-action" type="button" onClick={relaunchOnboarding}>Relancer onboarding</button></div></div>
      <div className="info-card"><h2>Admin PIN</h2><p>Default starting PIN is 0000. Change it here after first login.</p><div className="form-grid"><label className="field"><span>Admin PIN</span><input inputMode="numeric" pattern="[0-9]*" maxLength="4" type="password" value={ownerKey} onChange={(event) => updateSettingsField("ownerKey", fourDigitPin(event.target.value))} /></label></div><div className="action-row"><button className="secondary-action" type="button" onClick={() => updateSettingsField("ownerKey", "0000")}>Reset to 0000</button></div></div>

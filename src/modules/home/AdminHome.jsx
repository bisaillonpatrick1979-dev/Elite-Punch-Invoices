...
export default function AdminHome({ onNavigate }) {
  ...
  return (
    <section className="module-page chantier-home">
      ...
      <div className="chantier-readout-grid"><button className="chantier-readout money" type="button" onClick={() => onNavigate?.("invoices")}><span>REVENU AUJOURD’HUI</span><strong>{money(summary.todayAmount)}</strong><small>{summary.todayHours.toFixed(2)} h · {summary.todayPunchCount} session(s)</small></button><button className="chantier-readout time" type="button" onClick={() => onNavigate?.("punch")}><span>{activePunch ? activePunch.workerName : "STATUT"}</span><strong>{activePunch ? secondsToClock(workedSeconds) : "00:00:00"}</strong><small>● {statusText}</small></button></div>
      <div className="chantier-punch-stage"><button className={activePunch ? "chantier-punch-main active" : "chantier-punch-main"} type="button" onClick={() => onNavigate?.("punch")}><span className="chantier-punch-icon">◉</span><strong>{activePunch ? "OUVRIR PUNCH" : "PUNCH IN"}</strong><small>{activePunch ? activePunch.clientName || activePunch.jobName || "Punch actif" : "Démarrer une session"}</small></button><div className="chantier-ready-pill"><span />{statusText}</div></div>

...
export default function EntryGate() {
  ...
  const onboardingDone = Boolean(settings.onboardingDone);
  ...
  const completeOnboarding = () => updateAppData((currentData) => ({ ...currentData, settings: { ...currentData.settings, onboardingDone: true } }));
  ...
  if (!onboardingDone) {
    return (
      <section className="module-page home-entry-page">
        <div className="hero-card"><span className="status-pill">Elite Punch Invoice</span><h2>Onboarding</h2><p>Première configuration : choisis la langue et la province. Tu pourras modifier ça plus tard.</p></div>
        <div className="info-card"><h2>Configuration de départ</h2><div className="form-grid"><label className="field"><span>Langue</span><select value={settings.language || "fr"} onChange={(event) => updateSetting("language", event.target.value)}><option value="fr">Français</option><option value="en">English</option></select></label><label className="field"><span>Province</span><select value={settings.region || "Alberta"} onChange={(event) => updateSetting("region", event.target.value)}><option value="Alberta">Alberta</option><option value="British Columbia">British Columbia</option><option value="Saskatchewan">Saskatchewan</option><option value="Manitoba">Manitoba</option><option value="Ontario">Ontario</option><option value="Quebec">Quebec</option></select></label></div><div className="action-row"><button className="primary-action" type="button" onClick={completeOnboarding}>Continuer</button></div></div>
      </section>
    );
  }
  ...
  return (
    <section className="module-page home-entry-page">
      <div className="hero-card"><span className="status-pill">Elite Punch Invoice</span><h2>Accueil</h2><p>Connecte-toi comme Admin ou Employé avec ton PIN.</p></div>

...
function getWorkedMinutes(activePunch, nowISO = new Date().toISOString()) {
  ...
  return Math.max(0, grossMinutes - getBreakMinutes(activePunch.breaks || [], nowISO));
}
...
export default function Punch() {
  ...
  const stopPunch = () => {
    ...
    updateAppData((currentData) => {
      ...
      const result = buildInvoicesFromPunches({ punches: nextPunches, invoices: currentData.invoices || [], taxes: currentData.settings?.taxProfile?.taxes || [] });
  ...
  return (
    <section className="module-page">
      <div className="hero-card"><span className="status-pill">{activePunch ? (isOnBreak ? "On break" : "Punch active") : "Ready"}</span><h2>Punch in / Punch out</h2><p>Local punch with automatic open invoice creation.</p><div className="money-preview">{formatMoney(liveAmount, settings.currency || "CAD", settings.locale || "fr-CA")}</div><div className="mini-stats"><span>{minutesToHours(workedMinutes).toFixed(2)} h worked</span><span>{minutesToHours(breakMinutes).toFixed(2)} h break</span><span>{formatMoney(effectiveHourly, settings.currency || "CAD", settings.locale || "fr-CA")} / h</span>{activePunch && <span>Started {formatTime(activePunch.startedAt)}</span>}</div><div className="action-row">{!activePunch ? <button className="primary-action" type="button" onClick={startPunch}>Punch In</button> : <><button className="secondary-action" type="button" onClick={isOnBreak ? endBreak : startBreak}>{isOnBreak ? "Break Out" : "Break In"}</button><button className="primary-action" type="button" onClick={stopPunch}>Punch Out</button></>}</div></div>
      {!activePunch && <div className="info-card"><h2>Punch setup</h2><div className="form-grid"><label className="field"><span>Worker</span><select value={form.workerId} onChange={(event) => selectWorker(event.target.value)}>{workers.map((worker) => <option key={worker.id} value={worker.id}>{worker.name}</option>)}</select></label><label className="field"><span>Saved client</span><select value={form.clientId} onChange={(event) => selectClient(event.target.value)}><option value="">Manual / no saved client</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select></label><label className="field"><span>Pay type</span><select value={form.payType} onChange={(event) => updateField("payType", event.target.value)}><option value={PAY_TYPES.HOURLY}>Hourly</option><option value={PAY_TYPES.SQUARE_FOOT}>Square foot</option><option value={PAY_TYPES.FIXED}>Fixed price</option></select></label><label className="field"><span>Client / company</span><input value={form.clientName} onChange={(event) => updateField("clientName", event.target.value)} /></label><label className="field"><span>Address / job</span><input value={form.jobAddress} onChange={(event) => updateField("jobAddress", event.target.value)} /></label><label className="field"><span>Short job name</span><input value={form.jobName} onChange={(event) => updateField("jobName", event.target.value)} /></label>{form.payType === PAY_TYPES.HOURLY && <label className="field"><span>Hourly rate</span><input type="number" min="0" step="0.01" value={form.hourlyRate} onChange={(event) => updateField("hourlyRate", event.target.value)} /></label>}{form.payType === PAY_TYPES.SQUARE_FOOT && <><label className="field"><span>Square feet</span><input type="number" min="0" step="0.01" value={form.squareFeet} onChange={(event) => updateField("squareFeet", event.target.value)} /></label><label className="field"><span>Rate per sq ft</span><input type="number" min="0" step="0.01" value={form.squareFootRate} onChange={(event) => updateField("squareFootRate", event.target.value)} /></label></>}{form.payType === PAY_TYPES.FIXED && <label className="field"><span>Fixed amount</span><input type="number" min="0" step="0.01" value={form.fixedAmount} onChange={(event) => updateField("fixedAmount", event.target.value)} /></label>}<label className="field field-full"><span>Notes</span><textarea rows="3" value={form.notes} onChange={(event) => updateField("notes", event.target.value)} /></label></div></div>}
      <div className="info-card"><h2>Recent punches</h2>{(appData.punches || []).length === 0 ? <p>No saved punch yet.</p> : <div className="simple-list">{appData.punches.slice(0, 5).map((punch) => <div className="list-item" key={punch.id}><strong>{punch.workerName}</strong><span>{punch.clientName || "No client"}</span><span>{punch.workedHours.toFixed(2)} h | Break {minutesToHours(punch.breakMinutes || 0).toFixed(2)} h | {formatMoney(punch.amount, settings.currency || "CAD", settings.locale || "fr-CA")}</span><span>Invoice: {punch.invoiceStatus || "not_invoiced"}</span></div>)}</div>}</div>
    </section>
  );
}

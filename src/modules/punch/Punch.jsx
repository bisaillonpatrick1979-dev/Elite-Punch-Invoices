...
export default function Punch() {
  ...
  const selectWorker = (workerId) => {
    ...
    setForm((currentForm) => ({
      ...currentForm,
      workerId,
      hourlyRate: String(worker?.defaultHourlyRate ?? currentForm.hourlyRate),
      squareFootRate: String(worker?.defaultSquareFootRate ?? currentForm.squareFootRate)
    }));
  };
  ...
  const selectClient = (clientId) => {
    ...
    setForm((currentForm) => ({
      ...currentForm,
      clientId,
      clientName: client?.name || currentForm.clientName,
      jobAddress: client?.civicAddress || currentForm.jobAddress
    }));
  };
  ...
  const stopPunch = () => {
    ...
    if (activePunch.currentBreakStartedAt) {
      finalBreaks.push({ id: `break-${Date.now()}`, startedAt: activePunch.currentBreakStartedAt, endedAt, minutes: minutesBetween(activePunch.currentBreakStartedAt, endedAt) });
    }
    ...
    updateAppData((currentData) => ({ ...currentData, activePunch: null, punches: [completedPunch, ...(currentData.punches || [])] }));
  };
  ...
  return (
    <section className="module-page">
      <div className="hero-card">
        ...
        <p>Local punch with clients, break tracking and worker default rates.</p>
        <div className="money-preview">{formatMoney(liveAmount, settings.currency || "CAD", settings.locale || "fr-CA")}</div>
        <div className="mini-stats"><span>{minutesToHours(workedMinutes).toFixed(2)} h worked</span><span>{minutesToHours(breakMinutes).toFixed(2)} h break</span><span>{formatMoney(effectiveHourly, settings.currency || "CAD", settings.locale || "fr-CA")} / h</span>{activePunch && <span>Started {formatTime(activePunch.startedAt)}</span>}</div>
        <div className="action-row">{!activePunch ? <button className="primary-action" type="button" onClick={startPunch}>Punch In</button> : <><button className="secondary-action" type="button" onClick={isOnBreak ? endBreak : startBreak}>{isOnBreak ? "Break Out" : "Break In"}</button><button className="primary-action" type="button" onClick={stopPunch}>Punch Out</button></>}</div>
      </div>
      ...
      {!activePunch && (
        <div className="info-card">
          ...
          <div className="form-grid">
            <label className="field"><span>Worker</span><select value={form.workerId} onChange={(event) => selectWorker(event.target.value)}>{workers.map((worker) => <option key={worker.id} value={worker.id}>{worker.name}</option>)}</select></label>
            <label className="field"><span>Saved client</span><select value={form.clientId} onChange={(event) => selectClient(event.target.value)}><option value="">Manual / no saved client</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select></label>

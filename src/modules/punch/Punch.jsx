...
export default function Punch() {
  ...
  const clients = isOwner ? (appData.clients || []).filter((client) => client.active !== false) : [];
  ...
  useEffect(() => {
    if (!isOwner && sessionWorkerId && form.workerId !== sessionWorkerId) {
      ...
      setForm((currentForm) => ({ ...currentForm, workerId: sessionWorkerId, clientId: "", hourlyRate: String(worker?.defaultHourlyRate ?? currentForm.hourlyRate), squareFootRate: String(worker?.defaultSquareFootRate ?? currentForm.squareFootRate) }));
    }
  ...
  const confirmStartPunch = () => {
    ...
    const client = isOwner ? clients.find((item) => item.id === form.clientId) : null;
    updateAppData((currentData) => ({ ...currentData, activePunch: { id: `active-${Date.now()}`, workerId: forcedWorkerId, workerName: worker?.name || "Worker", clientId: isOwner ? form.clientId : "", clientName: form.clientName.trim(), clientPhone: client?.phone || "", clientEmail: client?.email || "", jobAddress: form.jobAddress.trim(), jobName: form.jobName.trim(), payType: form.payType, hourlyRate: Number(form.hourlyRate || 0), squareFeet: Number(form.squareFeet || 0), squareFootRate: Number(form.squareFootRate || 0), fixedAmount: Number(form.fixedAmount || 0), notes: form.notes.trim(), startedAt: new Date().toISOString(), breaks: [], currentBreakStartedAt: null } }));
